    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                ant_s=self.ant
                if ant_s.is_container==True and ant_s.can_contain(insect)==True:
                    insect.place=self
                    ant_s.contain_ant(insect)
                    ant_s=insect
                elif insect.can_contain(self.ant)==True and insect.is_container==True:
                    insect.place=self
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                # Bodyguard was d. Contained ant should remain in the game
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        place=self.place.ant
        if place==None or place.blocks_path==False:
            return False
        else:
            return True
        # END Problem 7
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        orig_arm=self.armor
        current=self.place
        total=self.damage+amount
        updated=self.armor-amount
        li_b=list(self.place.bees)
        if orig_arm>0 and amount>0:
            for i in li_b:
                if updated<=0:
                    i.reduce_armor(total)
                elif updated>0:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        b_arm=bee.armor
        Insect.reduce_armor(bee,b_arm)
        # END Problem 6

    def action(self, colony):
        # BEGIN Problem 6
        place_b=self.place.bees
        random_b=random_or_none(place_b)
        if self.digesting>0:
            self.digesting=self.digesting-1
        elif self.digesting<=0:
            if random_b !=None:
                self.eat_bee(random_b)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        dam_amt=self.damage
        [i.reduce_armor(dam_amt) for i in bees]
        # END Problem 7
...
class BodyguardAnt(Ant):
    ...
    armor=2
    implemented = True
    can_contain=True   # Change to True to view in the GUI
    # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.contained_ant:
            self.place=self.contained_ant.place
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        current1=self.place
        cont_ant=self.contained_ant
        if cont_ant!=None:
            current1=cont_ant.place
            cont_ant.action(colony)
        list_bees=list(current1.bees)
        for i in list_bees:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_watersafe==False:
            cur_arm=insect.armor
            insect.reduce_armor(cur_arm)
        # END Problem 11
